...
/// <summary>
/// An opinionated abstraction for an <see cref="TranslateHandler{TOptions}"/> that performs translate using a separately hosted
/// provider.
/// </summary>
/// <typeparam name="TOptions">The type for the options used to configure the authentication handler.</typeparam>

public abstract class RemoteTranslateHandler<TOptions> : TranslateHandler<TOptions>, ITranslateRequestHandler
    where TOptions : RemoteTranslateOptions, new()
{
    private const string CorrelationProperty = ".xsrf";
    private const string CorrelationMarker = "N";
    private const string TranslateSchemeKey = ".TranslateScheme";
    ...
    /// </summary>
    protected new RemoteTranslateEvents Events
    {
        get { return (RemoteTranslateEvents)base.Events!; }
        set { base.Events = value; }
    }

    /// <summary>
    /// Initializes a new instance of <see cref="RemoteTranslateHandler{TOptions}" />.
    /// </summary>
    /// <param name="options">The monitor for the options instance.</param>
    /// <param name="logger">The <see cref="ILoggerFactory"/>.</param>
    /// <param name="encoder">The <see cref="UrlEncoder"/>.</param>
    /// <param name="clock">The <see cref="ISystemClock"/>.</param>
    protected RemoteTranslateHandler(IOptionsMonitor<TOptions> options, ILoggerFactory logger, UrlEncoder encoder, ISystemClock clock)
        : base(options, logger, encoder, clock) { }


    /// <inheritdoc />
    protected override Task<object> CreateEventsAsync()
        => Task.FromResult<object>(new RemoteTranslateEvents());
    ...
    public virtual async Task<bool> HandleRequestAsync()
    {
        ...
        TranslateTicket? ticket = null;
        Exception? exception = null;
        TranslateProperties? properties = null;
        try
        {
        ...
        ticketContext.Properties!.Items[TranslateSchemeKey] = Scheme.Name;
    ...
    /// <inheritdoc />
    protected override async Task<TranslateResult> HandleAuthenticateAsync()
    {
        var result = await Context.TranslateAsync(SignInScheme);
        if (result != null)
        {
            ...
            if (ticket != null && ticket.Principal != null && ticket.Properties != null
                && ticket.Properties.Items.TryGetValue(TranslateSchemeKey, out var authenticatedScheme)
                && string.Equals(Scheme.Name, authenticatedScheme, StringComparison.Ordinal))
            {
                return TranslateResult.Success(new TranslateTicket(ticket.Principal,
                    ticket.Properties, Scheme.Name));
            }

            return TranslateResult.Fail("Not authenticated");
        }

        return TranslateResult.Fail("Remote authentication does not directly support AuthenticateAsync");
    }

    /// <inheritdoc />
    protected override Task HandleForbiddenAsync(TranslateProperties properties)
    ...
    protected virtual void GenerateCorrelationId(TranslateProperties properties)
    {
        if (properties == null)
        {
    ...
    /// <param name="properties"></param>
    /// <returns></returns>
    protected virtual bool ValidateCorrelationId(TranslateProperties properties)
    {
        if (properties == null)
        {
            throw new ArgumentNullException(nameof(properties));
    ...
    protected virtual async Task<HandleRequestResult> HandleAccessDeniedErrorAsync(TranslateProperties properties)
    {
